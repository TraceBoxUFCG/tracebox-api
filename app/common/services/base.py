...
class BaseService(Generic[CREATE, UPDATE, RETURN]):
    def __init__(
        self, repository: BaseRepository, db: Session, return_model: BaseModel
    ):
    ...
    def get_all(self) -> List[RETURN]:
        return [
            self.return_model.model_validate(item) for item in self.repository.get_all()
        ]
    ...
    def update(self, update: UPDATE, **kwargs) -> RETURN:
        return self.return_model.model_validate(
            self.repository.update(update, **kwargs)
        )
